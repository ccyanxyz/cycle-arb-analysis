...
class UniswapObject(object):
    ...
    def __init__(self, address, private_key, provider=None):
        ...
        self.conn = Web3(Web3.HTTPProvider(self.provider, request_kwargs={"timeout": 6000}))
...
class UniswapV2Client(UniswapObject):
    ...
    def set_sushi(self):
        ...
        self.contract = self.conn.eth.contract(
            address=Web3.toChecksumAddress(factory), abi=UniswapV2Client.ABI)
        self.router = self.conn.eth.contract(
            address=Web3.toChecksumAddress(router), abi=UniswapV2Client.ROUTER_ABI)
    def set_uni(self):
        self.contract = self.conn.eth.contract(
            address=Web3.toChecksumAddress(UniswapV2Client.ADDRESS), abi=UniswapV2Client.ABI)
        self.router = self.conn.eth.contract(
    ...
    def _is_approved(self, token, amount=MAX_APPROVAL_INT):
        ...
        approved_amount = erc20_contract.functions.allowance(self.address, self.router.address).call(block_identifier=self.block_number)
    ...
    def get_pair(self, token_a, token_b):
        ...
        return self.contract.functions.getPair(addr_1, addr_2).call(block_identifier=self.block_number)
    ...
    def get_pair_by_index(self, pair_index):
        ...
        """
        try:
            return self.contract.functions.allPairs(pair_index).call(block_identifier=self.block_number)
    ...
    def get_num_pairs(self):
        ...
        """
        return self.contract.functions.allPairsLength().call(block_identifier=self.block_number)

    def get_fee(self):
        """
        :return: Protocol wide fee.
        """
        return self.contract.functions.feeTo().call(block_identifier=self.block_number)

    def get_fee_setter(self):
        """
        :return: Address allowed to change the fee.
        """
        return self.contract.functions.feeToSetter().call(block_identifier=self.block_number)
    ...
    def get_factory(self, query_chain=False):
        ...
        """
        if query_chain:
            return self.router.functions.factory().call(block_identifier=self.block_number)
    ...
    def get_price_0_cumulative_last(self, pair):
        ...
        pair_contract = self.conn.eth.contract(
            address=Web3.toChecksumAddress(pair), abi=UniswapV2Client.PAIR_ABI)
        return pair_contract.functions.price0CumulativeLast().call(block_identifier=self.block_number)
    ...
    def get_price_1_cumulative_last(self, pair):
        ...
        pair_contract = self.conn.eth.contract(
            address=Web3.toChecksumAddress(pair), abi=UniswapV2Client.PAIR_ABI)
        return pair_contract.functions.price1CumulativeLast().call(block_identifier=self.block_number)
    ...
    def get_k_last(self, pair):
        ...
        pair_contract = self.conn.eth.contract(
            address=Web3.toChecksumAddress(pair), abi=UniswapV2Client.PAIR_ABI)
        return pair_contract.functions.kLast().call(block_identifier=self.block_number)

    def get_amounts_out(self, amountIn, path):
        router = self.conn.eth.contract(
                address=Web3.toChecksumAddress(UniswapV2Client.ROUTER_ADDRESS), abi=UniswapV2Client.ROUTER_ABI)
        return router.functions.getAmountsOut(amountIn, path).call(block_identifier = self.block_number)
