...
with open('data/cycle_include_router_with_tx_from1.json') as f:
    for line in f:
        ...
        if info['tx']['to'] and info['tx']['to'].lower() == "0x7a250d5630b4cf539739df2c5dacb4c659f2488d":
            router_count += 1
            if _from not in router_stats.keys():
                router_stats[_from] = {'count': 0, 'profit': 0, 'revenue': 0, 'cost': 0}
            router_stats[_from]['count'] += 1
            router_stats[_from]['revenue'] += info['revenue']
            router_stats[_from]['cost'] += info['cost']
            router_stats[_from]['profit'] += info['revenue'] - info['cost']
            continue
...
json.dump(router_stats, open('data/eoa_stats_router.json', 'w'))
